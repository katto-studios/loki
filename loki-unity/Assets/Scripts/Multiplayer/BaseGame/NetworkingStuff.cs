...
public class NetworkingStuff : MonoBehaviour, IPunCallbacks{
    [Header("Room stuff")] public InputField inCreateRoom;
    ...
    [Header("Gameplay")] public GameObject goToPlayGameScene;
    ...
    private void InitaliseTimeTrail(){
        if (PhotonNetwork.isMasterClient){
            int roomSeed = Random.Range(0, int.MaxValue);
            ...
            if (PhotonNetwork.room.CustomProperties["RandomSeed"] == null){
                //set room
                PhotonNetwork.room.SetCustomProperties(new Hashtable(){
                    {"RandomSeed", roomSeed},
                    {"ReadyToStart", true}
        ...
        if (inCreateRoom.text.Equals("")){
            ...
            if (!PhotonNetwork.CreateRoom(inCreateRoom.text, new RoomOptions(){
                ...
                PopupManager.Instance.ShowPopUp(
                    string.Format("Room {0} successfully created!", PhotonNetwork.room.Name), 3.0f);
            }
        }
    }
    ...
    public void OnJoinedRoom(){
        if (PhotonNetwork.room != null){
            ...
            PlayfabUserInfo.SetUserState(PhotonNetwork.room.PlayerCount == 1
                ? PlayfabUserInfo.UserState.InQueue
                : PlayfabUserInfo.UserState.ReadyToType);
